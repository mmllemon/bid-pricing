...
4. **成本税口径与隐形成本仍是正式产品化的主要缺口。** 当前把成本清单综合单价当作「含税成本单价」处理，进项税抵扣与项目固定成本分摊链路尚未完整产品化；模型利润不能直接当成最终财务利润。
...
| 成本综合单价 | 成本清单原始含税成本 `cost_input_incl_vat`；有效成本税务转换尚未完整产品化 |
...
- **成本税口径**：输入语义是「含税成本单价」，但进项税抵扣方式、有效成本转换、项目固定成本录入/分摊链路未完整闭环，模型利润不等于最终财务利润。
...
1. **成本税口径闭环**（P0）：进项税抵扣方式声明 + 有效成本转换 + 缺失时阻断利润结论。
