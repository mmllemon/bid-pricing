...
4. **成本税口径已闭环，取值已声明为 `PARTIAL`（H-002 / ADR-0035）。** 成本清单综合单价按「含税成本单价」入账，系统在**产出任何毛利数字之前**按已声明的抵扣方式换算为不含税有效成本；未声明抵扣方式时**阻断**「最优利润」结论（`config/project_quote_policy.json` 的 `cost_input_tax_policy.mode` 已于 2026-09-20 声明为 `PARTIAL`：成本构成两桶——材料设备 65%/13% + 劳务及措施 35%/9%，换算系数 k≈0.896322；**未声明时仍会阻断**——这是设计行为，不是故障）。模型利润的口径与单价、限价一致（均不含增值税），但**仍不是最终财务利润**：项目固定成本与隐性成本链路未闭环。
...
- **成本税口径取值已落（2026-09-20）**：`project_quote_policy.cost_input_tax_policy.mode` = `PARTIAL`，构成两桶（材料设备 65%/13% + 劳务及措施 35%/9%，k≈0.896322，派生可抵扣占比 1.0），前端「本次报价参数 · 成本税口径（成本构成）」面板支持逐项目自定义占比/税率并经 `cost_composition` 覆盖贯通 optimize/recompute。若各清单构成差异较大，仍建议扩展为逐项可抵扣占比（见 ADR-0035 遗留 1）。
...
1. ~~**成本税口径闭环**（P0）~~ **已实现（H-002 / ADR-0035，2026-09-20）**：抵扣方式声明 + 含税→有效成本换算（唯一入口）+ 声明缺失时在出数前阻断利润结论；两条报价入口共用同一判据，并新增 PB-07 跨层口径对账。**遗留**：项目取值已声明为 `PARTIAL`（两桶构成，2026-09-20）；若项目构成差异大，建议扩展为逐项占比。
