...
1. **项目取值未落**：`config/project_quote_policy.json` 的 `cost_input_tax_policy.mode`
   保持 `UNKNOWN` —— 抵扣方式与占比是**项目事实**，须由责任人声明，
   Agent 不得代填（这正是 H-002「阻断」的含义：声明前不出结论）。
   若各清单的货物/人工构成比例差异较大，单一全局 `credit_ratio` 会引入偏差，
   届时需扩展为**逐项可抵扣占比**（本 ADR 的换算式天然支持逐项 ρ）。
2. **未接线**：前端尚不能在计算前预告「本项目未声明成本税口径」，
   目前靠计算后阻断 + `user_hint` 提示（可接受，但可优化）。
